...
class linear_layer:
    ...
    def forward(self, X):
        ...
        # N = X.shape[0]
        # D = X.shape[1]
        # forward_output = np.zeros((N, D))
        # for i in range(N):
        #     forward_output[i] = np.matmul(X[i], self.params['W']).reshape(1, D)
        #     forward_output[i] += self.params['b']
        forward_output = np.dot(X, self.params['W'])
        forward_output += self.params['b']
...
class relu:
    ...
    def backward(self, X, grad):
        ...
        N = X.shape[0]
        D = X.shape[1]
        backward_output = np.multiply(grad, np.heaviside(X, 0))
        return backward_output
